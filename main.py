...
def _register_button_press():
    ...
    now = time.time()
    if now - _last_button_ts < BUTTON_DEBOUNCE_S: return False
    _last_button_ts = now; return True

def button_callback(pin):
    # (This function is UNCHANGED)
    global state
    if not _register_button_press(): return
    with state_lock:
        if state == 'idle': state = 'closing'; print("[BUTTON] Press -> start closing")
        elif state == 'closing': state = 'idle'; print("[BUTTON] Press -> abort/stop (open)")
    ...
    try:
        while not stop_flag.is_set():
            ...
            if s == 'idle':
                ...
                if not_open: servo_ctrl.smooth_open()
            ...
            if s == 'closing':
                ...
                while not all(reached.values()) and state == 'closing' and not stop_flag.is_set():
                    with latest_lock: fsr_vals = list(latest['fsr']); temp = latest['temp']
                    if temp >= TEMP_ABORT:
                    ...
                    time.sleep(STEP_DELAY)
                with state_lock:
                    if state == 'closing': state = 'gripped'; print("[STATE] Gripping done -> state = gripped")
                    else: print("[STATE] Closing interrupted; new state:", state)
            elif s == 'gripped':
                time.sleep(0.12)
            else:
                time.sleep(0.12)
...
# --- NEW: Task Thread for LEDs and Delay ---
def _led_task_thread_fn(category, pi_instance):
    """
    This function runs on its own thread to handle the 40-second
    delay and LED control, without blocking the main loop.
    """
    global g_task_running
    
    print(f"[Task Thread] Starting 40s task for category: '{category}'")
    
    try:
        if category == "soft":
            print("[Task Thread] Turning BLUE LED ON.")
            pi_instance.write(LED_BLUE_PIN, 1) # ON
            
            for _ in range(int(DELAY_SECONDS)):
                if stop_flag.is_set(): break
                time.sleep(1)
                
            pi_instance.write(LED_BLUE_PIN, 0) # OFF
            
        elif category == "hard":
            print("[Task Thread] Turning RED LED ON.")
            pi_instance.write(LED_RED_PIN, 1) # ON
            
            for _ in range(int(DELAY_SECONDS)):
                if stop_flag.is_set(): break
                time.sleep(1)
                
            pi_instance.write(LED_RED_PIN, 0) # OFF
        ...
        # Ensure LEDs are off
        try:
            pi_instance.write(LED_BLUE_PIN, 0)
            pi_instance.write(LED_RED_PIN, 0)
        except Exception:
            pass
        
        print("[Task Thread] Task finished. Awaiting next detection.")
        g_task_running = False # Signal that we are ready for a new task
...
def main():
    ...
    try:
        ...
        print("[MAIN] Failed to setup button via pigpio:", e); button_cb = None

    # Start actuation threads (UNCHANGED)
    ...
    try:
        while True:
            # --- NEW LOGIC START ---
            if not g_task_running: # Only check for a new task if one isn't already running
                try:
                    # Check the queue for a new category (non-blocking)
                    category = category_queue.get_nowait()
                    
                    print(f"\n[MAIN] New category received: '{category}'")
                    
                    # 1. Update the global FSR thresholds
                    if category == "soft":
                        THRESHOLDS = [100, 100, 100, 100, 100]
                        print("[MAIN] FSR thresholds set to SOFT (100)")
                    elif category == "hard":
                        THRESHOLDS = [200, 200, 200, 200, 200]
                        print("[MAIN] FSR thresholds set to HARD (200)")
            ...
            if not th_serial.is_alive():
                ...
                th_serial = threading.Thread(target=serial_thread_fn, args=(SERIAL_PORT, BAUDRATE), daemon=True)
                th_serial.start()
            if not th_fingers.is_alive():
                 print("[MAIN] ERROR: Finger thread died! Restarting...")
                 th_fingers = threading.Thread(target=finger_actuation_loop, args=(servo_ctrl, FINGER_PINS), daemon=True)
                 th_fingers.start()
            if not th_we.is_alive():
                 print("[MAIN] ERROR: Wrist/Elbow thread died! Restarting...")
                 th_we = threading.Thread(target=wrist_elbow_loop, args=(servo_ctrl, WRIST_ELBOW_PINS, ADC_CHANNELS), daemon=True)
                 th_we.start()
            if not th_vision.is_alive():
